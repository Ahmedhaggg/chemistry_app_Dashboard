                ...
                : <div className="dashboard-section">
                    <SectionHeader text="students" />
                    {
                        getGrades.data && <select className="form-select form-select-lg mb-3" aria-label="select select grade"
                            onChange={(e) => {
                                console.log(getGrades.data.grades[e.target.selectedIndex])
                                setGrade(e.target.value == "all" ? null : e.target.value)
                                setSearchParams({ 
                                    grade: e.target.value === "all" ? "all" : getGrades.data.grades[e.target.selectedIndex].name.replaceAll(" ", '_'),
                                    page: 1 
                                })
                            }}
                    >
                                ...
                                data.students.map(student => (
                                    <tr className="text-center" key={student._id}>
                                        <td data-title="student" className="fs-6">
                                            <Link className="text-black-color text-decoration-none" to={"/students/" + student._id}>{student.name}</Link>
                                        </td>
